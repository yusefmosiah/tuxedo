# Stellar Tools LangChain Integration Fix

## Problem Summary

The agent-first architecture is failing due to Pydantic v2 JSON schema generation errors when LangChain tries to convert Stellar tools to OpenAI function format. The root cause is custom class dependencies (`KeyManager`, `Server`) in function signatures that cannot be serialized to JSON schema.
...
1. **Custom Class Type Annotations**: Stellar tools use `KeyManager` and `Server` as parameter types
2. **Schema Generation Failure**: Pydantic v2 cannot serialize custom classes that aren't Pydantic models
3. **Docstring Mismatch**: Parameter descriptions don't match actual function signatures after modifications
...
- ✅ Follows LangChain architectural patterns
- ✅ Provides proper error handling and type safety

## Implementation Plan

### Step 1: Create Tool Wrappers Module

Create new file: `backend/agent/stellar_tools_wrappers.py`
...
### Step 2: Update Agent Tool Loading

Modify `backend/agent/core.py` to use the new wrappers:

```python
# Around line 68, replace the Stellar tools import section:
...
3. **Frontend Testing**:
   ...
   # Test Stellar queries in chat interface:
...
Test that the tools generate proper JSON schemas:

```python
# Create test script: test_tool_schemas.py
from langchain_core.utils.json_schema import dereference_refs
from agent.stellar_tools_wrappers import stellar_account_manager

# Test schema generation
schema = stellar_account_manager.args_schema
...
try:
    ...
    print("❌ Schema serialization failed:", e)
```

### Step 5: Clean Up (Optional)

Once the implementation is verified to work:

1. **Remove Original Tool Import**: Remove the old Stellar tools import from `backend/agent/core.py`
2. **Update Documentation**: Update any references to the old tool names
3. **Add Tests**: Create unit tests for the new wrapper functions

## Implementation Checklist

- [ ] Create `backend/agent/stellar_tools_wrappers.py`
- [ ] Implement all 6 tool wrappers with clean schemas
- [ ] Update `backend/agent/core.py` tool imports
- [ ] Test schema generation for all tools
- [ ] Run `test_agent_with_tools.py` to verify functionality
- [ ] Test chat endpoint with Stellar queries
- [ ] Verify frontend integration works
- [ ] Clean up old imports and documentation

## Expected Outcome

After implementation:

1. **Schema Generation**: All tools will generate clean JSON schemas without Pydantic errors
2. **Functionality**: All existing Stellar features will continue to work
3. **Architecture**: Clean separation between LangChain and business logic
4. **Maintainability**: Easier to modify and extend in the future
...
1. **Import Errors**: Ensure all module paths are correct in imports
2. **Async/Sync Mismatch**: The soroban wrapper handles async operations properly
3. **Environment Variables**: Verify all required environment variables are set
4. **Type Validation**: Ensure all wrapper parameters use primitive types only
...
# Check tool loading
cd backend && python -c "
from agent.core import load_agent_tools
import asyncio
...
result = stellar_utilities('status')
print(result)
"
```
...
If you need a faster but less clean solution, you can temporarily remove the problematic type annotations:

```python
# In stellar_tools.py and stellar_soroban.py, change:
def account_manager(action: str, key_manager: KeyManager, horizon: Server, ...):

# To:
def account_manager(action: str, key_manager, horizon, ...):
```

This resolves the immediate error but loses type safety and is not recommended for production code.

---

**Note**: This implementation follows LangChain best practices and maintains the existing architecture while solving the Pydantic v2 compatibility issues. The wrapper approach provides a clean separation between the AI agent interface and the Stellar blockchain operations.