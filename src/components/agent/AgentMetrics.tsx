...
export const AgentMetrics: React.FC = () => {
  ...
  const totalBalance = agent.accounts.reduce(
      ...
      >
        ...
        <div style={{ color: "var(--color-text-secondary)", lineHeight: 1.5 }}>
          {agent.status === "active" ? (
            <>
              {" "}
              🤖 Agent is <strong>actively managing</strong> {activeAccounts}{" "}
              account(s) with a total balance of {totalBalance.toFixed(2)} XLM.
            </>
          ) : agent.status === "idle" ? (
            <>
              {" "}
              🔄 Agent is <strong>idle</strong> and ready for new tasks. Start a
              conversation to activate the agent.
            </>
          ) : (
            <>
              {" "}
              ⚠️ Agent is in <strong>error state</strong>. Please check the
              error message and try refreshing.
            </>
          )}
          <br />
          <br />
